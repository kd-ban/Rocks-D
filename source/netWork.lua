...
if opPCL == IOS_TEST_ZH then             -- ios 测试
    HostTable.DEFAULT_HOST = "http://188.188.188.106/pirate/public"    -- 外网测试服务器
...
--------- MobGame 越南 ios ---------
elseif opPCL == IOS_VIETNAM_VI or opPCL == ANDROID_VIETNAM_VI or opPCL == ANDROID_VIETNAM_VI2 then         
    -- HostTable.DEFAULT_HOST = "http://188.188.188.108/pirate/public"    -- 外网测试服务器
    -- HostTable.SSO_DEFAULT_HOST = "http://188.188.188.108/pirate/public" -- 外网sso服务器
    HostTable.CDN_ROOT_OP2DXUP = "http://dthkcdn.kiemvuongchimong.com/" -- cdn地址

    HostTable.DEFAULT_HOST = "http://192.168.8.59/pirate/public"    -- 外网测试服务器
    HostTable.SSO_DEFAULT_HOST = "http://192.168.8.59/pirate/public/" -- 外网sso服务器
--------- MobGame 印度尼西亚 ios ---------
elseif opPCL == IOS_MOB_INDONISIA
    or opPCL == ANDROID_MOB_INDONISIA  then         
    HostTable.DEFAULT_HOST = "http://op.in.7talent.com/public"    -- 外网测试服务器
    HostTable.SSO_DEFAULT_HOST = "http://op.in.7talent.com/public/" -- 外网sso服务器
...
--------- MobGame 越南 android ---------
-- elseif opPCL == ANDROID_VIETNAM_VI or opPCL == ANDROID_VIETNAM_VI2 then
...
elseif opPCL == IOS_TGAME_SPAIN then
    HostTable.DEFAULT_HOST = "http://op.sp.7talent.com/public/"    -- 外网测试服务器
    HostTable.SSO_DEFAULT_HOST = "http://op.sp.7talent.com/public/" -- 外网sso服务器
    HostTable.CDN_ROOT_OP2DXUP = "http://op.sp.7talent.com/admin" -- cdn地址
end
    ...
    function(res, hnd)
        ...
        if rtnData["code"] ~= 200 and rtnData["code"] ~= ErrorCodeTable.ERR_1207 then
           -- print(" Print By lixq ---- ErrorCodeTable.ERR_1207asdklfjlksdfjklfslkjfadsjkl")
            if callbackErrorFun ~= nil then
                callbackErrorFun(url, rtnData["code"], rtnData.info)
            end
            return
        end
        --callbackFun
...
-- 公开调用方法
function doActionNoLoadingFun( actionType, params, callBackFun, errorFun, serverCode, isSSO )
    -- body
    if not serverCode and userdata then
        serverCode = userdata.serverCode
    end
    doAction(actionType, serverCode, params, callBackFun, errorFun, nil, isSSO)
end
