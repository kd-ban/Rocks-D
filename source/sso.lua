...
function SSOPlatform.Login()
	...
	-- Auto Guest: create a guest UID automatically on first launch
	local autoGuestUid = SSOPlatform.GetUid()
	if autoGuestUid == nil or string.len(autoGuestUid) <= 0 then
	...
	if SSOPlatform.GetUid() and string.len(SSOPlatform.GetUid()) > 0 then			-- 非首次登陆
		...
		if isPlatform(IOS_VIETNAM_VI) 
        	or isPlatform(IOS_MOB_INDONISIA) 
            ...
            or isPlatform(ANDROID_VIETNAM_VI2)
            ...
            or isPlatform(ANDROID_MOB_INDONISIA)  then
